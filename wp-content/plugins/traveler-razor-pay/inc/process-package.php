...
if(!class_exists('RazorProcessPackage')){
    class RazorProcessPackage{
        ...
        public function __construct(){
            ...
            add_action('st_receipt_st_razor_package', array($this, 'receipt_page'),10,1);
        ...
        public function receipt_page($orderId)
            ...
            $order = $cls_packages->get_order_by_id($orderId);
            try
            {
        ...
        private function getDefaultCheckoutArguments($order)
        {
            ...
            $cls_packages = STPackages::get_inst();
            $infor_odder = unserialize($cls_packages->get_order_by_id($orderId)->partner_info);
            ...
            return array(
                ...
                'notes'        => array(
                    ...
                    __('Full Name','razor-pay') => $infor_odder['firstname'] . " ". $infor_odder['lastname'],
                    __('Email','razor-pay') => $infor_odder['email'],
                    __('Phone','razor-pay') => $infor_odder['phone'],
        ...
        {
            $cls_packages = STPackages::get_inst();
            $infor_odder = unserialize($cls_packages->get_order_by_id($order->id)->partner_info);
            $args = array(
                'name'    => $infor_odder['firstname'] . " ". $infor_odder['lastname'],
                'email'   => $infor_odder['email'],
                'contact' => $infor_odder['phone'],
            );
        ...
        private function getOrderCreationData($orderId)
        {
            ...
            $pay_amount = !empty($order->package_price) ? intval($order->package_price) : 0;
            $payment_action_razor = st()->get_option('payment_action_razor','');
            $data = array(
                'receipt'         => $orderId,
                'amount'          => (int) round($pay_amount * 100),
        ...
        function _st_booking_change_status($status,$order_id,$booking_type)
        {
            ...
            global $wpdb;
            $table_name=$wpdb->prefix . 'st_member_packages_order';
            $sql="UPDATE $table_name SET `status`='$status' where `id` = $order_id";
            $wpdb->query($sql);
        }

		function check_razorpay_response()
        {    if(get_page_template_slug(get_the_ID()) === 'template-package-success-new.php'){
                $order_token_code=STInput::get('order_token_code');
                if($order_token_code)
                {
                    $admin_packages = STAdminPackages::get_inst();
                    $cls_packages = STPackages::get_inst();
                    $get_order_by_token = $cls_packages->get_order_by_token($order_token_code);

                    $order_code_id=$get_order_by_token->id;

                }
                $gateway=$get_order_by_token->gateway;
                if($gateway==='st_razor'){
                    global $wp_session;
                    $orderId = !empty($_SESSION[self::SESSION_KEY]) ? $_SESSION[self::SESSION_KEY] : '';
                    if ($get_order_by_token->status !== 'completed' ) {
                        ...
                        $sessionKey = $this->getOrderSessionKey($orderId);
                        $razorpay_order_id = !empty($_SESSION[$sessionKey]) ? $_SESSION[$sessionKey] : '';
                        if (!empty($orderId)  && !empty($_POST[self::RAZORPAY_PAYMENT_ID]))
                        {
                            ...
                            $this->updateOrder($get_order_by_token, $success, $error, $razorpayPaymentId,$orderId);
                        }
                        else
                        {
        ...
        public function updateOrder($order, $success, $errorMessage, $razorpayPaymentId, $orderId)
        {
            ...
            if ($order->status !== 'completed' ) {
                ...
                if ($success === true)
                {
                    $this->_st_booking_change_status('completed',$idOrder, 'normal_booking');
