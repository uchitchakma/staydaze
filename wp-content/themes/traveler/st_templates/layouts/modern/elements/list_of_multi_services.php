			...
			<div class="st-list-dropdown">
				...
				<ul class="list">
					...
					foreach ( $list_services as $k => $v ) {
						...
						echo "<li data-value='" . esc_attr( $v['service'] ) . "' data-arg ='" . str_ireplace( [ "'" ], '\"', balanceTags( wp_json_encode( $args_list ) ) ) . "' class='" . esc_attr( $class ) . "'>" . esc_html( $v['name'] ) . '</li>';
						++$i;
					}
					?>
				</ul>
			...
			switch ( $v['service'] ) {
				case 'st_hotel':
					if ( st_check_service_available( 'st_hotel' ) ) {
						...
						while ( $query->have_posts() ) :
							$query->the_post();
							$html .= st()->load_template( 'layouts/modern/hotel/loop/grid', '', [ 'slider' => true ] );
						endwhile;
						$hotel->remove_alter_search_query();
						wp_reset_postdata();
						$post  = $old_post;
				...
				case 'st_activity':
					if ( st_check_service_available( 'st_activity' ) ) {
						...
						if ( $query->have_posts() ) {
							...
							while ( $query->have_posts() ) :
								$query->the_post();
								echo st()->load_template( 'layouts/modern/activity/elements/loop/grid', '', [ 'slider' => true ] );
							endwhile;
							echo '</div></div></div>';
						}
				...
				case 'st_rental':
					if ( st_check_service_available( 'st_rental' ) ) {
						...
						while ( $query->have_posts() ) :
							$query->the_post();
							$html .= st()->load_template( 'layouts/modern/rental/elements/loop/grid', '', [ 'slider' => true ] );
						endwhile;
				...
				case 'st_cars':
					if ( st_check_service_available( 'st_cars' ) ) {
						...
						while ( $query->have_posts() ) :
							$query->the_post();
							$html .= st()->load_template( 'layouts/modern/car/elements/loop/grid', '', [ 'slider' => true ] );
						endwhile;
