...
class STInstall
{
    ...
    function st_check_page_url(){
		...
		if($current_slug->base === 'traveler_page_st-vina-extensions'){
			...
			wp_localize_script('st_install_plugin_js', 'st_install_plugin_localize', [
				'nonce' => wp_create_nonce('st_install_plugin')
			]);
		}
    }
    ...
    public function st_install_plugin(){
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			echo json_encode( [
				'status'  => 'error',
				'message' => __( 'You are not allowed to install plugin', 'traveler' )
			] );
			die;
		}

		if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'st_install_plugin') ) {
			echo json_encode( [
				'status'  => 'error',
