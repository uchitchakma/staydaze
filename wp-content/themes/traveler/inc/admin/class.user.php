    ...
    class STUser extends STAdmin
    {
        ...
        public function st_notifyauthor_callback( $post){
            ...
            if (!empty( $author->roles ) && in_array( 'partner' , $author->roles ) && st()->get_option('partner_post_by_admin', 'on') == 'on') {
                ...
                wp_mail( $author->user_email, $subject, $message, $headers, false );
        ...
        static function add_edit_scripts()
        {
            ...
            wp_localize_script('jquery', 'st_params_partner', [
				...
				'_s'           => wp_create_nonce( 'st_backend_security' ),
				'nonce' => wp_create_nonce('change_status_withdrawal')
            ]);
        ...
        static function _load_more_service_partner($post_type = false, $user_id = false)
        {
            if (STInput::request('st_post_type', '') != '') {
                $post_type = esc_html( STInput::request('st_post_type') );
            }
            if (STInput::request('st_user_id', '') != '') {
                $user_id = esc_html( STInput::request('st_user_id') );
            }
            if (!empty($post_type) and !empty($user_id)) {
				...
				$querystr = $wpdb->prepare(
					...
					WHERE 1 = 1
						AND {$wpdb->prefix}posts.post_author = %d
						AND {$wpdb->prefix}posts.post_type = %s
					ORDER BY {$wpdb->prefix}posts.post_date DESC
					LIMIT %d, %d",
					$user_id, $post_type, $offset, $limit
				);
        ...
        public function _sendmail_expire_partner()
        {
			if ( ! current_user_can( 'manage_options' ) ) {
				echo json_encode( [
					'status'  => 'error',
					'message' => __( 'You are not allowed to send mail', 'traveler' )
				] );
				die;
			}

			if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'sendmail_expire_partner' ) ) {
				echo json_encode( [
					'status'  => 'error',
