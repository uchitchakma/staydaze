    ...
    if ( !class_exists( 'STAdminUpdateContent' ) ) {
        ...
        class STAdminUpdateContent extends STAdmin
        {
            ...
            function _add_style_javascript()
            {
				...
				wp_localize_script( 'update-content.js', 'update_content_localize', [
					'nonce' => wp_create_nonce( 'update_content_func' ),
                ] );
            }
            ...
            static function st_my_update_content_func()
            {
				if ( ! current_user_can( 'manage_options' ) ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed to update content', 'traveler' )
					] );
					die;
				}

				if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'update_content_func' ) ) {
					echo json_encode( [
						'status'  => 'error',
