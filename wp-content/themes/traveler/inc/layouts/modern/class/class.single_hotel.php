...
class ST_Single_Hotel extends TravelerObject {
	...
	public function __singleRoomFilterAjax() {
		...
		if ( have_posts() ) {
			...
			while ( have_posts() ) {
				...
				if ( ! empty( $_GET['version'] ) && ( $_GET['version'] == 'elementorv2' ) && ( in_array( 'traveler-layout-essential-for-elementor/traveler-layout-essential-for-elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ) {
					if ( $layout_val == 'grid' ) {
						...
						$item_row              = $_GET['st_item_row'];
						$item_row_tablet       = $_GET['st_item_row_tablet'];
						$item_row_tablet_extra = $_GET['st_item_row_tablet_extra'];
						if ( $item_row ) {
							$col_classes = 'col-12 col-sm-' . ( 12 / $item_row_tablet ) . ' col-md-' . ( 12 / $item_row_tablet_extra ) . ' col-lg-' . ( 12 / $item_row );
						}
						$res .= '<div class="' . esc_attr( $col_classes ) . '">';
						$res .= apply_filters( 'ste_load_template', ste_loadTemplate( 'list-service-room/room/loop/grid' ) );
					} elseif ( $layout_val == 'list' ) {
			...
			if ( ! empty( $_GET['version'] ) && ( $_GET['version'] == 'elementorv2' ) && ( in_array( 'traveler-layout-essential-for-elementor/traveler-layout-essential-for-elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ) {
				...
				$res .= '<div class="panigation-list-new-style pagination moderm-pagination" data-action_service="sts_filter_room_ajax" data-layout="' . esc_attr( $layout_val ) . '" data-st_item_row="' . $item_row . '" data-st_item_row_tablet="' . $item_row_tablet . '" data-st_item_row_tablet_extra="' . $item_row_tablet_extra . '" data-order="' . esc_attr( $st_order ) . '" data-orderby="' . esc_attr( $st_orderby ) . '">';
	...
	public function setQueryRoomSearch() {
		...
		if ( ! empty( $_GET['post__in'] ) ) {
			$post_ids_room = $_GET['post__in'];
		}

		if ( $_GET['st_orderby'] === 'post__in' && ! empty( $post_ids_room ) ) {
	...
	public function __changePostField( $fields ) {
		...
		if ($disable_avai_check == 'off') {
			...
			$fields .= ', SUM(CAST(CASE WHEN IFNULL(tb.adult_price, 0) = 0 THEN tb.price ELSE tb.adult_price END AS DECIMAL)) as st_price';
		}
		return $fields;
	}
