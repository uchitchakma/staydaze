...
class ST_Menu_Mega_New
{
    ...
    public function st_nav_update($menu_id, $menu_item_db_id)
    {
        ...
        $menumega_enabled_value = $_POST['menu_item_megamenu'][$menu_item_db_id];
        update_post_meta($menu_item_db_id, '_menu_item_megamenu', $menumega_enabled_value);

        if (isset($menumega_enabled_value) && !empty($_POST['menu_item_megamenu_columns'])) {
            $menumega_columns_enabled_value = $_POST['menu_item_megamenu_columns'][$menu_item_db_id];
            update_post_meta($menu_item_db_id, '_menu_item_megamenu_columns', $menumega_columns_enabled_value);
        }

        if (!isset($_POST['menu_item_menutitle'][$menu_item_db_id])) {
            $_POST['menu_item_menutitle'][$menu_item_db_id] = '';
        }
        $menutitle_enabled_value = $_POST['menu_item_menutitle'][$menu_item_db_id];
        update_post_meta($menu_item_db_id, '_menu_item_menutitle', $menutitle_enabled_value);

        if (!empty($_POST['menu_item_menulabel'])) {
            $menulabel_enabled_value = $_POST['menu_item_menulabel'][$menu_item_db_id];
            update_post_meta($menu_item_db_id, '_menu_item_menulabel', $menulabel_enabled_value);
        }

        if (!empty($_POST['menu_item_menulabelcolor'])) {
            $menulabelcolor_enabled_value = $_POST['menu_item_menulabelcolor'][$menu_item_db_id];
            update_post_meta($menu_item_db_id, '_menu_item_menulabelcolor', $menulabelcolor_enabled_value);
        }

        if (!empty($_POST['menu_item_menuimage'])) {
            $menuimage_enabled_value = $_POST['menu_item_menuimage'][$menu_item_db_id];
