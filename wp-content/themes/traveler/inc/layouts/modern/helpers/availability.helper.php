        ...
        public function insert_calendar_bulk($data, $posts_per_page, $total, $current_page, $all_days, $post_id)
        {
            ...
            if ($data['groupday'] == 0) {
                for ($i = $start; $i <= $end; $i++) {
                    ...
                    /*  .End */
                    $this->traveler_insert_availability($data['post_id'], $data['start'], $data['end'], $data['price'], $data['adult_price'], $data['children_price'], $data['infant_price'], $data['starttime'], $count_starttime, $data['status'], $data['groupday'], $number_booked, $table);
                }
            } else {
                for ($i = $start; $i <= $end; $i++) {
