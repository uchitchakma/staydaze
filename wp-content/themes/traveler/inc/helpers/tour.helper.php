    ...
    if ( !class_exists( 'TourHelper' ) ) {
        class TourHelper
        {
            ...
            static function _get_disable_date()
            {
                ...
                $tour_id = esc_html( STInput::request( 'tour_id', '' ) );
                if ( empty( $tour_id ) ) {
                    echo json_encode( $disable );
                    die();
                }
                ...
                $year = esc_html( STInput::request( 'year', date( 'Y' ) ) );
				...
				$sql = $wpdb->prepare(
					...
					WHERE
						...
						AND YEAR(FROM_UNIXTIME(check_in_timestamp)) = %d
						AND YEAR(FROM_UNIXTIME(check_out_timestamp)) = %d
					",
					$tour_id, $year, $year
				);
            ...
            static function _tourValidate( $check_in, $check_out )
            {
	            $data = ST_Tour_Availability::inst()
		                                        ...
		                                        check_in BETWEEN {$check_in} AND {$check_out} OR check_out BETWEEN {$check_in} AND {$check_out} END", null, true)
	                                            ->where("`status`", 'available')
	                                            ->where("number_booked < (count_starttime * CASE WHEN number > 0 THEN `number` else (number_booked + 1) END)", true, false)
	                                            ->get()->result();
	            $data_id = "''";
