            ...
            function _get_where_query( $where )
            {
                ...
                if ( isset( $st_search_args[ 'location_id' ] ) && !empty( $st_search_args[ 'location_id' ] ) ) {
                    ...
                    if ( !empty( $ids_location ) && is_array( $ids_location ) ) {
						...
						$where .= $wpdb->prepare( " AND (tb2.address LIKE %s", '%' . $location_name . '%' );
                ...
                if ( isset( $_GET[ 'start' ] ) && !empty( $_GET[ 'start' ] ) && isset( $_GET[ 'end' ] ) && !empty( $_GET[ 'end' ] ) ) {
                    ...
                    $adult_number = esc_html( STInput::get( 'adult_number', 0 ) );
                    if ( intval( $adult_number ) < 0 ) $adult_number = 0;

                    $children_number = esc_html( STInput::get( 'children_num', 0 ) );
                    if ( intval( $children_number ) < 0 ) $children_number = 0;

                    $number_room = esc_html( STInput::get( 'room_num_search', 0 ) );
                    ...
                    if ( !is_array( $list_hotel ) || count( $list_hotel ) <= 0 ) {
                        ...
                        if ( !empty( $list_hotel ) ) {
                            $check_in_rewhere  = esc_html( STInput::get( 'start', '' ) );
                            $check_out_rewhere = esc_html( STInput::get( 'end', '' ) );
                            if ( !empty( $check_in_rewhere ) || !empty( $check_out_rewhere ) ) {
								$where .= $wpdb->prepare( " AND {$wpdb->prefix}posts.ID NOT IN (%s)", $list_hotel );
                            }
                        }
                    }

					$where .= $wpdb->prepare( " AND CAST(tb2.hotel_booking_period AS UNSIGNED) <= %s", $period );

					$where .= $wpdb->prepare( " AND tb.check_in >= %s AND tb.check_out <= %s", $check_in_stamp, $check_out_stamp );
                ...
                if ( isset( $_REQUEST[ 'star_rate' ] ) && !empty( $_REQUEST[ 'star_rate' ] ) ) {
                    ...
                    if ( $list_star ) {
						$where .= $wpdb->prepare( " AND (tb2.rate_review IN (%s)", $list_star );
                    }
                }

                if ( isset( $_REQUEST[ 'hotel_rate' ] ) && !empty( $_REQUEST[ 'hotel_rate' ] ) ) {
                    $hotel_rate = esc_html( STInput::get( 'hotel_rate', '' ) );
					$where .= $wpdb->prepare( " AND (tb2.hotel_star IN (%s))", $hotel_rate );
...
//            }
                ...
                if ( !empty( $_REQUEST[ 'taxonomy_hotel_room' ] ) ) {
                    $tax = esc_html( STInput::request( 'taxonomy_hotel_room' ) );
                    if ( !empty( $tax ) and is_array( $tax ) ) {
                        ...
                        if ( !empty( $tax_query ) ) {
							...
							foreach ($tax_query as $k => $v) {
								$placeholders = implode(',', array_fill(0, count($v['terms']), '%d'));
								$conditions[] = "(
									SELECT COUNT(1)
								...
								$params = array_merge($params, $v['terms']);
								$params[] = count($v['terms']);
							}

							$where_room .= implode(' AND ', $conditions) . " )";
							$where_room = $wpdb->prepare($where_room, ...$params);
                    ...
                    if ( $st_search_args[ 'only_featured_location' ] == 'yes' and is_array( $featured ) ) {
                        ...
                        if ( is_array( $featured ) && count( $featured ) ) {
							...
							foreach ($featured as $item) {
								$where_conditions[] = "tb2.multi_location LIKE %s";
								$params[] = "%_{$item}_%";
							}

							$placeholders = implode(',', array_fill(0, count($featured), '%d'));
							$where_conditions[] = "tb2.id_location IN ($placeholders)";
