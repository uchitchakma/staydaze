...
jQuery(function ($) {
    ...
    $('body').on('change', '.woocs_price_col .wc_input_price, .chosen_select', function () {
...
function woocs_add_group_geo(post_id, hash) {
    ...
    jQuery('#woocs_li_geo_' + hash + '_' + index + ' .woocs_country_select').chosen();
    jQuery('#woocs_li_geo_' + hash + '_' + index + ' .woocs_country_select').trigger("chosen:updated");
    //***
    woocs_enable_save_variation_changes();
}
...
function woocs_set_geoip_profile(id) {

    var data = jQuery('#' + id).find('.woocs_geoip_profile').val();
    if (data = JSON.parse(data)) {
        jQuery('#' + id).find('select.woocs_country_select').val(data);

        jQuery('#' + id).find('select.woocs_country_select').trigger('change');
        jQuery('#' + id).find('select.woocs_country_select').trigger("chosen:updated");
